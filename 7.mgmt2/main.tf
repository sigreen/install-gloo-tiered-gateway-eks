...
locals {
  ...
  mgmt2cluster_additional_sg_id = data.terraform_remote_state.vpc.outputs.mgmt2cluster_additional_sg_id
...
module "eks" {
  ...
  eks_managed_node_groups = {
    mgmtcluster = {
      ...
      max_size               = 5
      desired_size           = 2
      vpc_security_group_ids = [local.mgmt2cluster_additional_sg_id]
...
resource "helm_release" "gloo-platform" {
  ...
  depends_on = [
    kubernetes_namespace.gloo-mesh,
    helm_release.gloo-platform-crds
  ]
}